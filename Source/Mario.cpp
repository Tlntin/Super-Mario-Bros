...
Mario::Mario() :
	...
	walk_animation(MARIO_WALK_ANIMATION_SPEED, CELL_SIZE, resourcePath() + "Images/MarioWalk.png")
{
	texture.loadFromFile(resourcePath() + "Images/MarioIdle.png");
...
void Mario::die()
{
	...
	texture.loadFromFile(resourcePath() + "Images/MarioDeath.png");
    // Load a music to play
    if (!music.openFromFile(resourcePath() + "audio/die.wav")) {
        return EXIT_FAILURE;
    }
...
void Mario::start_music(){
    if (!music.openFromFile(resourcePath() + "audio/bgm.wav")) {
        return EXIT_FAILURE;
    }
    music.setLoop(true);
    music.play();
...
void Mario::draw(sf::RenderWindow& i_window)
{
	...
	if (0 == dead)
	{
		if (0 == on_ground)
		{
			texture.loadFromFile(resourcePath() + "Images/MarioJump.png");
		}
		else
		{
			...
			{
				texture.loadFromFile(resourcePath() + "Images/MarioBrake.png");
			}
			else
			{
				draw_sprite = 0;
